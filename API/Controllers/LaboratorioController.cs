...
[ApiVersion("1.1")]
    public class LaboratorioController : BaseApiController
    {
        ...
        public async Task<ActionResult<Pager<LaboratorioDto>>> LaboratorioPaginacion([FromQuery] Params hamb_ingParams)
        {
            var Laboratorios = await _unitOfWork.Laboratorios.GetAllAsync(hamb_ingParams.PageIndex,hamb_ingParams.PageSize,hamb_ingParams.Search);
            var ListLaboratorios=_mapper.Map<List<LaboratorioDto>>(Laboratorios.registros);

            return new Pager<LaboratorioDto>(ListLaboratorios,Laboratorios.totalRegistros,  hamb_ingParams.PageIndex, hamb_ingParams.PageSize,hamb_ingParams.Search);
        }
